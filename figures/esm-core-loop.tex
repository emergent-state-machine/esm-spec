\begin{tikzpicture}[
  scale=0.92,
  transform shape,
  node distance=0.68cm,
  box/.style={
    rectangle,
    rounded corners,
    draw,
    align=center,
    minimum width=10.2cm,
    minimum height=0.92cm,
    font=\small
  },
  smallbox/.style={
    rectangle,
    rounded corners,
    draw,
    align=center,
    minimum width=8.0cm,
    minimum height=0.84cm,
    font=\small
  },
  note/.style={
    align=left,
    font=\small
  },
  arrow/.style={
    -{Latex[length=2.1mm]},
    thick
  }
]

% Main vertical chain
\node[box] (obs) {\textbf{Observations}\\Raw measurements or events from the monitored environment};

\node[box, below=of obs] (sig) {\textbf{Signals}\\Direct and derived signals computed deterministically from observations};

\node[smallbox, below=of sig] (state) {\textbf{State Vector}\\Coherent situational state (fully resolved; all signals interpreted together)};

\node[smallbox, below=of state] (proj) {\textbf{Projection into Policy Space}\\State re-expressed in policy-relevant coordinates (no new semantic content)};

\node[smallbox, below=of proj] (gate) {\textbf{Relevance Determination (Gating)}\\Condition determining whether policy evaluation begins};

\node[smallbox, below=of gate] (policy) {\textbf{Policy and Authorized Outcome}\\Deterministic evaluation of the next authorized response};

\node[smallbox, below=of policy] (outcome) {\textbf{Outcome}\\Turn result: no-op, action, escalation, handoff, or clarification};

\node[smallbox, below=of outcome] (action) {\textbf{Action (when authorized)}\\Executed intervention, alert, prompt, or workflow step};

\node[smallbox, below=of action] (evolve) {\textbf{State Evolution}\\New observations arrive; next turn begins};

% Main arrows
\draw[arrow] (obs) -- (sig);
\draw[arrow] (sig) -- (state);
\draw[arrow] (state) -- (proj);
\draw[arrow] (proj) -- (gate);
\draw[arrow] (gate) -- (policy);
\draw[arrow] (policy) -- (outcome);
\draw[arrow] (outcome) -- (action);
\draw[arrow] (action) -- (evolve);

% Gate side note for no-op
\node[note, right=0.8cm of gate, yshift=0.12cm] (gatenote) {If not relevant: \textbf{no-op}\\(no policy action; state still evolves)};
\draw[arrow] (gate.east) -- ++(0.7,0) |- (outcome.east);

% Loop back
\draw[arrow] (evolve.east) .. controls +(2.6,0) and +(2.6,0) .. (obs.east);

% Loop label
\node[note, right=2.55cm of policy] {Next turn with\\new observations};

% Bottom summary text
\node[align=center, below=0.45cm of evolve, font=\small] (traj)
{\textbf{Trajectory through State Space:} $x_1 \rightarrow x_2 \rightarrow x_3 \rightarrow \cdots$\\[0.15em]
\textbf{Local decisions shape the trajectory}: $x_1 \to x_2 \to \dots$ toward policy-defined regions};

\end{tikzpicture}
